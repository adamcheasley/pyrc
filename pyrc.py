...
Extended into pyrc by:
(c) 2009 Adam Cheasley
...
class IRC:
  def __init__(self, svr="80.68.90.35", prt=6667, nck="PythIRC", rname="Python-IRC User"):
    self.server = svr
    self.port = prt
    self.nick = nck
    self.realname = rname
  ...
  def parsecmd(self, input):
    ...
    if input[0] == "/" :
      ...
      elif output == 'JOIN' or output == 'J' :
        # Only supports one channel, no keys, at this time
        if parsedcmd.group(3) is not None:
          output = 'JOIN' + " " + parsedcmd.group(3) # Note - group(2) contains that space
...
if (__name__ == "__main__"):
  ...
  if len(sys.argv) == 5:
    client = IRC(svr=sys.argv[1], prt=int(sys.argv[2]), nck=sys.argv[3], rname=sys.argv[4])
    cursesui = CursesUI(client)
    # Event loop happens ... need more graceful way
    cursesui.close()
  elif len(sys.argv) > 1 and len(sys.argv) < 5:
    print "add some more arguments here"
  else:
    usage()
